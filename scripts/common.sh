...
# ── 确保工具链可用（WebUI exec 环境 PATH 可能不完整）──
export PATH="/sbin:/system/sbin:/system/bin:/system/xbin:${PATH:-/sbin:/system/bin}"
...
parse_rules() {
    ...
    while IFS= read -r _raw; do
        ...
        # 注意：/system/bin/sh 脚本模式下 | 在模式匹配里有特殊含义，必须转义
        _src="${_line%%\|*}";  _rest="${_line#*\|}"
        _pat="${_rest%%\|*}";  _dst="${_rest#*\|}"

        # 内联 canon_path，避免 $() 子shell在不同sh实现下的继承差异
        _p="${_src%/}"
        case "$_p" in
            /sdcard)         _src="/storage/emulated/0" ;;
            /sdcard/*)       _src="/storage/emulated/0/${_p#/sdcard/}" ;;
            /mnt/sdcard)     _src="/storage/emulated/0" ;;
            /mnt/sdcard/*)   _src="/storage/emulated/0/${_p#/mnt/sdcard/}" ;;
            /data/media/0)   _src="/storage/emulated/0" ;;
            /data/media/0/*) _src="/storage/emulated/0/${_p#/data/media/0/}" ;;
            *)               _src="$_p" ;;
        esac

        _p="${_dst%/}"
        case "$_p" in
            /sdcard)         _dst="/storage/emulated/0" ;;
            /sdcard/*)       _dst="/storage/emulated/0/${_p#/sdcard/}" ;;
            /mnt/sdcard)     _dst="/storage/emulated/0" ;;
            /mnt/sdcard/*)   _dst="/storage/emulated/0/${_p#/mnt/sdcard/}" ;;
            /data/media/0)   _dst="/storage/emulated/0" ;;
            /data/media/0/*) _dst="/storage/emulated/0/${_p#/data/media/0/}" ;;
            *)               _dst="$_p" ;;
        esac
